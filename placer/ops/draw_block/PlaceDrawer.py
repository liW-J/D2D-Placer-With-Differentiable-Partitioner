...
class PlaceDrawer(object):
    ...
    def forward(pos,
        ...
        # safe conversion helper: works for torch tensors, numpy arrays, lists
        def to_numpy(arr):
            try:
        ...
        x = to_numpy(pos[:num_nodes])
        y = to_numpy(pos[num_nodes:])
        node_size_x = to_numpy(node_size_x)
        node_size_y = to_numpy(node_size_y)
        pin_offset_x = to_numpy(pin_offset_x)
        pin_offset_y = to_numpy(pin_offset_y)
        pin2node_map = to_numpy(pin2node_map)
        # normalize tier to numpy array if provided (supports torch tensor/list)
        if tier is not None:
            try:
                if hasattr(tier, 'detach'):
                    tier = tier.detach().cpu().numpy()
                else:
                    tier = np.array(tier)
            except Exception:
                tier = np.array(tier)
        try:
            ...
            if len(node_xl) > num_physical_nodes:  # filler is included
                ...
                for i in range(num_physical_nodes, num_nodes):
                    draw_rect(node_xl[i], node_yl[i], node_xh[i], node_yh[i])
            # draw movable cells; color by tier if provided (0->blue, 1->green)
            ctx.set_line_width(line_width * 2)
            for i in range(num_movable_nodes):
                if tier is not None and i < len(tier) and int(tier[i]) == 1:
                    fill_rgba = (0.0, 0.6, 0.0, 0.5)
                    edge_rgba = (0.0, 0.4, 0.0, 0.8)
                else:
                    fill_rgba = (0.0, 0.0, 1.0, 0.5)
                    edge_rgba = (0.0, 0.0, 0.8, 0.8)
