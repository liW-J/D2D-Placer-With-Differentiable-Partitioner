...
from placer.ops.draw_block.PlaceDrawer import PlaceDrawer

class DrawBlockFunction(Function):
    @staticmethod
    def forward(
            pos, 
...
class DrawBlock(object):
    ...
    def forward(self, pos, filename, tier=None): 
        """ 
        @param pos cell locations, array of x locations and then y locations 
        @param filename suffix specifies the format 
        """
        return DrawBlockFunction.forward(
                pos, 
                self.node_size_x, 
                self.node_size_y, 
                self.pin_offset_x, 
    ...
    def __call__(self, pos, filename, tier):
        """
        @brief top API 
        @param pos cell locations, array of x locations and then y locations 
        @param filename suffix specifies the format 
        """
        return self.forward(pos, filename, tier)
