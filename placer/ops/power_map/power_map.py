...
LastEditTime: 2025-04-08 12:23:18
FilePath: /D2D-placer/placer/ops/power_map/power_map.py
...
import placer.ops.power_map.power_map_cpp as power_map_cpp
import torch
...
class PowerMapFunction(Function):
    ...
    @staticmethod
    def forward(pos, node_size_x, node_size_y, initial_power_map, xl, yl, xh,
                yh, num_bins_x, num_bins_y, range_begin, range_end,
                deterministic_flag, power):

        func = power_map_cpp.power_map
        output = func(pos.view(pos.numel()), node_size_x, node_size_y,
                      initial_power_map, xl, yl, xh, yh, num_bins_x,
                      num_bins_y, range_begin, range_end, deterministic_flag,
                      power)
        return output
...
class PowerMap(object):
    ...
    def forward(self, pos):
        ...
        if self.initial_power_map is None:
            self.initial_power_map = torch.zeros(self.num_bins_x,
                                                 self.num_bins_y,
                                                 dtype=pos.dtype,
                                                 device=pos.device)
