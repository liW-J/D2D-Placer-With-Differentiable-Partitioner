...
class MacroBalanceFunction(Function):
    ...
    def forward(tier, flat_netpin, netpin_start, pin2node_map, net_weights,
        ...
        output = func(tier,
                      ...
                      pin_pos, pos_terminal_legalized,
...
class MacroBalance(object):
    ...
    def __call__(self,
                 ...
                 terminal_names=[]):
        output = MacroBalanceFunction.forward(
            tier.cpu().contiguous(), self.flat_netpin_cpu,
            self.netpin_start_cpu, self.pin2node_map_cpu,
            self.net_weights_cpu, self.num_movable_nodes, self.node_size_x_cpu,
            ...
            pin_pos.cpu().contiguous(), pos_terminal_legalized.cpu().contiguous(),
            num_terminals, self.node_names, self.net_names, terminal_names,
            pos_2d.cpu().contiguous(), self.case_name, node_orient,
            movable_macro_mask.cpu().contiguous())
