...
import ops.weighted_average_wirelength_3d.weighted_average_wirelength_3d_cpp as weighted_average_wirelength_3d_cpp
import ops.weighted_average_wirelength_3d.weighted_average_wirelength_3d_cpp_atomic as weighted_average_wirelength_3d_cpp_atomic
import ops.weighted_average_wirelength_3d.weighted_average_wirelength_3d_cpp_merged as weighted_average_wirelength_3d_cpp_merged

logger = logging.getLogger(__name__)

class WeightedAverageWirelength3DFunction(Function):
    """
    ...
    def forward(ctx, pos, flat_netpin, netpin_start, pin2net_map, net_weights,
        ...
        """
        tt = time.time()
        func = weighted_average_wirelength_3d_cpp.forward
    ...
    def backward(ctx, grad_pos):
        tt = time.time()
        func = weighted_average_wirelength_3d_cpp.backward
...
class WeightedAverageWirelengthAtomic3DFunction(Function):
    """
    @brief compute weighted average wirelength.
    """
    @staticmethod
    def forward(ctx, pos, pin2net_map, flat_netpin, netpin_start, net_weights,
        ...
        """
        tt = time.time()
        func = weighted_average_wirelength_3d_cpp_atomic.forward
    ...
    def backward(ctx, grad_pos):
        tt = time.time()
        func = weighted_average_wirelength_3d_cpp_atomic.backward
        output = func(grad_pos, ctx.pos, ctx.exp_xy.view([-1]),
                      ctx.exp_nxy.view([-1]), ctx.exp_xy_sum.view([-1]),
                      ctx.exp_nxy_sum.view([-1]), ctx.xyexp_xy_sum.view([-1]),
...
class WeightedAverageWirelengthMerged3DFunction(Function):
    """
    @brief compute weighted average wirelength.
    """
    @staticmethod
    def forward(ctx, pos, flat_netpin, netpin_start, pin2net_map, net_weights,
        ...
        """
        tt = time.time()
        func = weighted_average_wirelength_3d_cpp_merged.forward
        output = func(pos.view(pos.numel()), flat_netpin, netpin_start,
                      pin2net_map, net_weights, net_mask, inv_gamma)
        ctx.pin2net_map = pin2net_map
        ctx.flat_netpin = flat_netpin
    ...
    def backward(ctx, grad_pos):
        tt = time.time()
        func = weighted_average_wirelength_3d_cpp_merged.backward
...
class WeightedAverageWirelength3D(nn.Module):
    """
    @brief Compute weighted average wirelength.
    ...
    def __init__(self,
        ...
        """
        super(WeightedAverageWirelength3D, self).__init__()
    ...
    def forward(self, pos):
        if self.algorithm == 'net-by-net':
            # TODO: 3D
            return WeightedAverageWirelength3DFunction.apply(
                pos,
                self.flat_netpin,
                self.netpin_start,
                self.pin2net_map,
            ...
            # TODO: 3D
            return WeightedAverageWirelengthAtomic3DFunction.apply(
                pos,
                self.pin2net_map,
                self.flat_netpin,
                self.netpin_start,
            ...
            return WeightedAverageWirelengthMerged3DFunction.apply(
                pos,
                self.flat_netpin,
                self.netpin_start,
                self.pin2net_map,
